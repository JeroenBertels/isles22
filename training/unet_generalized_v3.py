...
Compared to unet_generalized_v2.py now the common pathway is also a model and applied multiple times in case of number_siam_pathways leading to a number of outputs equal to the number of all possible permutations of the inputs
"""
...
def create_generalized_unet_v3_model(
    ...
    def retrieve_extra_output(path, i):
        ...
        for j, (e_o_k_s, e_o_n_f, e_o_d) in enumerate(zip(extra_output_kernel_sizes[i], extra_output_number_features[i], extra_output_dropout[i])):
            ...
            extra_output_path = Conv3D(filters=e_o_n_f, kernel_size=e_o_k_s, activation=extra_output_activation_final_layer[i] if j + 1 == len(extra_output_number_features[i]) else None, padding=padding, kernel_initializer=kernel_initializer, kernel_regularizer=regularizers.l1_l2(l1_reg, l2_reg))(extra_output_path)
    ...
    if number_siam_pathways > 1:
        ...
        for i in range(number_siam_pathways - 1):
            ...
            if dynamic_input_shapes:
                ...
                input__ = Input(shape=list(input_size) + [number_input_features], name=f"siam{i + 1}_input")

            inputs_.append(input__)
            ...
            paths.append(siam_model(inputs_ if len(inputs_) > 1 else inputs_[0]))

    # 2. Construct common pathway
    # first construct a separate model with its own input
    c_p_inputs = []
    number_features_start_common_pathway = number_features_per_pathway[0][-1][-1] * number_siam_pathways
    if dynamic_input_shapes:
        path = c_p_input = Input(shape=(None, None, None, number_features_start_common_pathway,), name="c_p_input")

    else:
        path = c_p_input = Input(shape=K.int_shape(path)[1:-1] + (number_features_start_common_pathway,), name="c_p_input")

    c_p_inputs.append(c_p_input)
    outputs = [None] * (len(extra_output_at_common_pathway_layer) + 1)
    for i, (n_f_c_p, k_s_c_p, d_c_p) in enumerate(zip(number_features_common_pathway, kernel_sizes_common_pathway, dropout_common_pathway)):
        ...
        for j, e_o_a_c_p_l in enumerate(extra_output_at_common_pathway_layer):
            if e_o_a_c_p_l == i:
                outputs[j + 1] = retrieve_extra_output(path, j)
    ...
    outputs[0] = path
    c_p_inputs += metadata_inputs
    c_p_model = Model(inputs=c_p_inputs, outputs=outputs)
    inputs += metadata_inputs
    outputs = []
    # now we must feed through this model all permutations of the concatenated outputs of the previous part
    nb_permutations = 0
    for perms in itertools.permutations(range(number_siam_pathways)):
        path = Concatenate(axis=-1)([paths[perm] for perm in perms]) if len(paths) > 1 else paths[0]
        path = c_p_model([path, *metadata_inputs])
        outputs += path if isinstance(path, list) else [path]
        nb_permutations += 1
    ...
    for i, output in enumerate(outputs):
        outputs[i] = LeakyReLU(1, name=f"s{i}")(output)
